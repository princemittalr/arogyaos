...
export class HealthScoreService {
  static async calculateScore(input: HealthScoreInput): Promise<HealthScoreResult> {
    try {
      const responseText = await callGemini(hospitalHealthPrompt, JSON.stringify(input));
      return JSON.parse(responseText) as HealthScoreResult;
    } catch (error) {
      console.warn('HealthScoreService falling back to mock due to:', error);
      
      let baseScore = 100;
      const factors: HealthScoreResult['factors'] = [];

      // Occupancy factor
      if (input.bedOccupancyPercent > 90) {
        baseScore -= 15;
      ...
      // Stockouts factor
      if (input.medicineStockoutPercent > 10) {
        baseScore -= 20;
