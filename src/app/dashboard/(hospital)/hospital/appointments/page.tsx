...
import {
  ...
  useUpdateAppointmentStatusMutation } from
...
export default function HospitalAppointmentsPage() {const { t } = useLanguage();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("hospital.consultation_roster")}
        description={t("hospital.monitor_daily_tokens_reschedule_appointments_and_register_patient_check_ins")} />
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filtered.length === 0 ?
        <p className="text-center text-xs text-slate-500 py-10 col-span-full">{t("hospital.no_appointments_in_this_category")}</p> :
            ...
            <motion.div
              ...
              className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 flex flex-col justify-between">
                ...
                <div>
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-[10px] font-extrabold text-blue-600 dark:bg-blue-950/20 dark:text-blue-400">{t("hospital.token")}
                      {appt.tokenNumber}
                      </span>
                      <p className="text-[9px] text-slate-400 mt-1 font-semibold uppercase tracking-wider">{t("hospital.id")}{appt.appointmentId}</p>
                    </div>
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-850 flex gap-2 justify-end">
                  ...
                  className="rounded-xl border border-slate-200 px-3 py-1.5 text-[10px] font-bold text-slate-500 hover:bg-slate-50 flex items-center gap-1">
                      ...
                      <Edit className="h-3 w-3" />{t("hospital.reschedule")}
                </button>
                    <button
                  ...
                  className="rounded-xl bg-emerald-600 hover:bg-emerald-700 px-3 py-1.5 text-[10px] font-bold text-white flex items-center gap-1">
                      ...
                      <CheckCircle className="h-3 w-3" />{t("hospital.complete")}
                </button>
                    <button
                  ...
                  className="rounded-xl border border-red-200 text-red-650 hover:bg-red-50 px-3 py-1.5 text-[10px] font-bold flex items-center gap-1">
                      ...
                      <XCircle className="h-3 w-3" />{t("hospital.cancel")}
                </button>
                  </div>
              }
              </motion.div>);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            className="relative z-10 w-full max-w-sm rounded-2xl border border-slate-250 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-850 pb-2 mb-4">
                <h3 className="font-extrabold text-sm text-slate-900 dark:text-slate-50">{t("hospital.reschedule_consultation")}</h3>
              ...
              <form onSubmit={handleSubmit(handleRescheduleSubmit)} className="space-y-4 text-xs">
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  <button
                  type="submit"
                  className="rounded-xl bg-blue-600 px-5 py-2 text-white font-bold hover:bg-blue-750">{t("hospital.apply_reschedule")}
