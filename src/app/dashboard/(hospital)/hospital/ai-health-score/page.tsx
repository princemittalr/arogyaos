...
export default function HospitalAIHealthScorePage() {const { t } = useLanguage();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("hospital.ai_hospital_health_score")}
        description={t("hospital.gemini_ai_calculates_a_0100_operational_health_score_based_on_bed_occupancy_medicine_availability_attendance_and_active_alerts")} />
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 space-y-5">
          <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
            <Activity className="h-4.5 w-4.5 text-blue-500" />{t("hospital.hospital_metrics_input")}
          </h3>

          <div className="space-y-5 text-xs font-bold text-slate-800 dark:text-slate-200">
            {([
            { key: 'bedOccupancyPercent', label: t("hospital.bed_occupancy"), min: 0, max: 100 },
            { key: 'medicineStockoutPercent', label: t("hospital.medicine_stockout"), min: 0, max: 100 },
            { key: 'staffAttendancePercent', label: t("hospital.staff_attendance"), min: 0, max: 100 },
            { key: 'activeAlertsCount', label: t("hospital.active_alerts_count"), min: 0, max: 20 },
            { key: 'appointmentsCount', label: t("hospital.appointments_today"), min: 0, max: 500 }] as
            const).map(({ key, label, min, max }) =>
          ...
          <button
            ...
            {isLoading ?
            <><Loader2 className="h-4 w-4 animate-spin" />{t("hospital.computing_score")}</> :

            <><Sparkles className="h-4 w-4" />{t("hospital.calculate_ai_health_score")}</>
            }
          </button>
        </div>
        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          <>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-3">
                  <h4 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50">{t("hospital.factor_breakdown")}</h4>
                ...
                <div key={idx} className="py-3 flex justify-between items-start gap-4">
                  ...
                  factor.impactScore >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`
                          ...
                          {factor.impactScore >= 0 ? '+' : ''}{factor.impactScore}{t("hospital.pts")}
                  </span>
                      </div>
                )}
                  </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-3">
                  <h4 className="font-extrabold text-xs uppercase tracking-wider text-emerald-600 flex items-center gap-1.5">
                    <Sparkles className="h-3.5 w-3.5" />{t("hospital.ai_recommendations")}
              </h4>
                  <div className="space-y-2">
          ...
          <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 dark:border-slate-800 dark:bg-slate-900 flex flex-col items-center gap-3 text-center">
              ...
              <p className="text-xs font-bold text-slate-600 dark:text-slate-400">{t("hospital.configure_metrics_and_run_the_ai_assessment")}</p>
            </div>
          }
        </div>
      </div>
