...
import {
  ...
  HospitalInventoryDocument } from
...
export default function InventoryPage() {const { t } = useLanguage();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("hospital.inventory_manager")}
        description={t("hospital.oversee_stockpiles_check_low_stock_alarms_track_suppliers_and_configure_procurement_limits")} />
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl border border-slate-200 dark:bg-slate-900 dark:border-slate-800">
        <div className="flex flex-col sm:flex-row w-full sm:w-auto items-center gap-3">
          ...
          <select
            ...
            <option value="">{t("hospital.all_categories")}</option>
            <option value="Analgesics">{t("hospital.analgesics")}</option>
            <option value="Antibiotics">{t("hospital.antibiotics")}</option>
            <option value="Cardiovascular">{t("hospital.cardiovascular")}</option>
            <option value="Vitamin Supplements">{t("hospital.vitamin_supplements")}</option>
          </select>
        </div>
        ...
        <button
          ...
          <span>{t("hospital.add_stock_item")}</span>
        </button>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-850 text-slate-450 font-bold">
                <th className="pb-3">{t("hospital.medicine_id")}</th>
                <th className="pb-3">{t("hospital.name")}</th>
                <th className="pb-3">{t("hospital.category")}</th>
                <th className="pb-3">{t("hospital.qty_min")}</th>
                <th className="pb-3">{t("hospital.expiry_date")}</th>
                <th className="pb-3">{t("hospital.supplier")}</th>
                <th className="pb-3">{t("hospital.stock_level")}</th>
            ...
            className="relative z-10 w-full max-w-md rounded-2xl border border-slate-250 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-xs">
                ...
                <div className="flex justify-end gap-3 pt-4">
                  ...
                  <button
                  type="submit"
                  className="rounded-xl bg-blue-600 px-5 py-2 text-white font-bold hover:bg-blue-750">{t("hospital.save_stock_item")}
