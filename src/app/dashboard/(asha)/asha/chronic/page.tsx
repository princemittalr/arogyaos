...
export default function AshaChronicPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useAshaModuleData(workerId, 'chronic');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Monitor medication adherence and home vitals for chronic conditions like Hypertension and TB." 
      />
      {!data || data.length === 0 ? (
        ...
        <EmptyState 
          ...
          description="All chronic disease follow-ups are up to date."
        />
      </div>
      ) : (
