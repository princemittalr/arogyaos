...
export default function AshaSettingsPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useAshaModuleData(workerId, 'settings');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Settings" 
        description="Manage your language preferences, theme, and notifications." 
      />
      {!data || data.length === 0 ? (
        ...
        <EmptyState 
          ...
          description="Your application preferences are up to date."
        />
      </div>
      ) : (
