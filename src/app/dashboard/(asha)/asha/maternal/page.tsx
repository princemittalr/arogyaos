...
export default function AshaMaternalPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useAshaModuleData(workerId, 'maternal');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Maternal Care" 
        description="Record antenatal and postnatal care logs including vitals and nutrition counseling." 
      />
      {!data || data.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
        <EmptyState 
          icon={icons.HeartPulse}
          title="No Maternal Records"
          description="No recent maternal care records found. Ensure vitals are updated during household visits."
