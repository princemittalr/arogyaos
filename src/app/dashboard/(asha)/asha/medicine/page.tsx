...
export default function AshaMedicinePage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useAshaModuleData(workerId, 'medicine');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Track your current inventory and log distributions to beneficiaries." 
      />
      {!data || data.length === 0 ? (
        ...
        <EmptyState 
          ...
          description="Your medical kit is empty. Request a restock from your nearest PHC."
