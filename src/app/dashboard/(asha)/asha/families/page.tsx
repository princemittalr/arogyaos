...
export default function AshaFamiliesPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useAshaModuleData(workerId, 'families');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Directory of all families and households assigned to your coverage area." 
      />
      {!data || data.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
        <EmptyState 
          icon={icons.Users}
          title="No Assigned Families"
          description="You currently have no families assigned. When households are assigned to your coverage area, they will appear here."
