...
export default function AshaEducationPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useAshaModuleData(workerId, 'education');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Plan community awareness sessions and distribute educational materials." 
      />
      {!data || data.length === 0 ? (
        ...
        <EmptyState 
          ...
          description="There are no upcoming health education campaigns scheduled for your area."
