...
export default function AshaChildHealthPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useAshaModuleData(workerId, 'child-health');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Track growth charts, nutrition, and upcoming vaccine schedules." 
      />
      {!data || data.length === 0 ? (
        ...
        <EmptyState 
          ...
          description="There are no pending immunizations for children in your coverage area."
