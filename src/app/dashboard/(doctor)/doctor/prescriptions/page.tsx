...
export default function DoctorPrescriptionsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Clinical Prescriptions Archives"
        description="Search past diagnostic recommendations, audit active treatment courses, or export medical prescription records."
      />
      ...
      {filteredRx.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="text-sm font-bold text-slate-800 dark:text-slate-100">No prescriptions found</p>
          <p className="text-xs text-slate-450 mt-1">Prescriptions will appear here once you complete patient consultations.</p>
        </div>
      ) : (
                ...
                <div className="space-y-3.5">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-semibold">Diagnosis: {rx.diagnosis}</p>
                    </div>
                  ...
                  <div className="rounded-xl bg-slate-50 dark:bg-slate-950 p-3 text-[11px] border border-slate-100 dark:border-slate-900 space-y-1.5 font-bold text-slate-650 dark:text-slate-400">
                    <span className="text-[10px] font-black uppercase text-slate-450 tracking-wider block mb-1">Medicines list</span>
                    {rx.medicines.map((med, idx) => (
                      <div key={idx} className="flex justify-between">
                        <span>{med.name}</span>
                        <span>{med.dosage} ({med.duration} days)</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {rx.labTests && rx.labTests.length > 0 && (
                    <div className="space-y-1">
                      <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Recommended Tests</span>
