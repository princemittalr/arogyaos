...
export default function DoctorAISummaryPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="AI Clinical Consultation Summary"
        description="Enter your clinical notes and let Gemini AI generate a structured diagnosis and prescription draft."
      />
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
                <ClipboardList className="h-4.5 w-4.5 text-blue-500" /> Clinical Notes Input
              </h3>
              <button
                onClick={() => setClinicalNotes(SAMPLE_NOTES)}
                className="text-[9px] font-bold text-blue-500 hover:text-blue-650 transition"
              >
                Load Sample
              </button>
            </div>
            ...
            <textarea
              ...
              placeholder="Enter clinical notes, symptoms, vitals, observations..."
              rows={10}
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl border border-dashed border-slate-200 dark:border-slate-800 text-slate-400">
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-600 dark:text-slate-400">Voice Transcript</p>
                <p className="text-[9px] text-slate-400 font-semibold">Voice-to-text integration coming in Phase 5</p>
              </div>
            </div>
        ...
        <div className="space-y-4">
          {summaryMutation.isPending && (
            <div className="rounded-2xl border border-slate-200 bg-white p-8 dark:border-slate-800 dark:bg-slate-900 space-y-4">
              <div className="flex items-center gap-3 text-xs text-slate-450 font-semibold">
                ...
                Gemini AI is generating your clinical summary...
              </div>
          ...
          {!summaryMutation.isPending && !result && (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-10 dark:border-slate-800 dark:bg-slate-900 flex flex-col items-center justify-center text-center gap-3">
              ...
              <p className="text-xs font-bold text-slate-600 dark:text-slate-400">AI Summary will appear here</p>
              <p className="text-[10px] text-slate-400 font-semibold max-w-xs">Enter clinical notes and click Generate to get a structured diagnosis and prescription draft.</p>
            </div>
          )}
          ...
          {result && (
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-3">
                <div className="flex justify-between items-center">
                  <h4 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
                    <MessageSquare className="h-4 w-4 text-blue-500" /> Clinical Summary
                  </h4>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-3">
                <h4 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
                  <Stethoscope className="h-4 w-4 text-emerald-500" /> Diagnosis
                </h4>
              ...
              {result.prescriptionDraft?.length > 0 && (
                <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-3">
                  <div className="flex justify-between items-center">
                    <h4 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
                      <Pill className="h-4 w-4 text-purple-500" /> Prescription Draft
                    </h4>
                    <span className="text-[9px] font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/20 px-2 py-0.5 rounded">
              ...
              {result.followUpAdvice && (
                <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-2">
                  <h4 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50">
                    Follow-up Advice
                  </h4>
