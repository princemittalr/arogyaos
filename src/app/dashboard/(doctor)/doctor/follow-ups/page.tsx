...
export default function DoctorFollowUpsPage() {const { t } = useLanguage();
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("doctor.patient_re_visit_registry")}
        description={t("doctor.monitor_recovery_progression_milestones_chronic_condition_follow_ups_or_check_in_schedules")} />
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="text-sm font-bold text-slate-800 dark:text-slate-100">{t("doctor.no")}{activeTab}{t("doctor.follow_ups_scheduled")}</p>
          <p className="text-xs text-slate-450 mt-1">{t("doctor.re_visit_reminders_scheduled_during_consultations_will_appear_here")}</p>
        </div> :
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div key={fup.followUpId} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 flex flex-col justify-between gap-4">
              <div className="space-y-3.5">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-semibold">{t("doctor.follow_up_id")}{fup.followUpId}</p>
                    </div>
                  </div>
                ...
                {fup.notes &&
            <p className="text-[10px] text-slate-500 italic bg-slate-50 dark:bg-slate-950 p-2.5 rounded-lg">{t("doctor.notes")}
              {fup.notes}
                  </p>
            }
              </div>
