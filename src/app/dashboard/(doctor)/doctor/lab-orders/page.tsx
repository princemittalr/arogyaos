...
export default function DoctorLabOrdersPage() {const { t } = useLanguage();
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("doctor.lab_test_recommendations")}
        description={t("doctor.verify_status_updates_for_diagnostic_requests_blood_profiles_or_radiology_recommendations")} />
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="text-sm font-bold text-slate-800 dark:text-slate-100">{t("doctor.no")}{activeTab}{t("doctor.lab_orders_found")}</p>
          <p className="text-xs text-slate-450 mt-1">{t("doctor.recommended_lab_tests_from_consultations_will_appear_here")}</p>
        </div> :
          ...
          return (
            <div key={order.orderId} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 flex flex-col justify-between gap-4">
                <div className="space-y-3.5">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 font-semibold">{t("doctor.order_id")}{order.orderId}</p>
                    </div>
                ...
                <span className="text-amber-600 dark:text-amber-400 flex items-center gap-1">
                      <AlertCircle className="h-3.5 w-3.5" />{t("doctor.sample_collection_pending")}
                </span> :
                order.status === 'completed' ?
                <span className="text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                      <CheckCircle className="h-3.5 w-3.5" />{t("doctor.diagnostic_panel_released")}
                </span> :

                <span className="text-slate-400 flex items-center gap-1">{t("doctor.order_cancelled")}
