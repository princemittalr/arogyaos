  ...
  if (!appointment) {
    return (
      <div className="p-8 text-center bg-white rounded-2xl border border-slate-200 dark:bg-slate-900 dark:border-slate-800">
        ...
        <h3 className="font-extrabold text-sm text-slate-900 dark:text-slate-50">Appointment Not Found</h3>
        <p className="text-xs text-slate-500 mt-1">This appointment token does not match your active patient queue.</p>
        <Link href="/dashboard/doctor" className="mt-4 inline-block text-xs font-bold text-blue-600 underline">
          Back to Desk
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-150 dark:border-slate-850 text-xs font-bold text-slate-650 dark:text-slate-400">
        <p>Age: {appointment.patientAge} years</p>
        <p className="capitalize">Gender: {appointment.patientGender}</p>
        <p>Blood Group: {appointment.patientBloodGroup || 'O+'}</p>
        <p>Allergies: {appointment.patientAllergies?.join(', ') || 'None'}</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 space-y-5">
            <h3 className="font-extrabold text-sm text-slate-900 dark:text-slate-50 border-b border-slate-100 dark:border-slate-850 pb-3">
              EMR Chart Record
            </h3>
            ...
            <div className="flex items-center justify-between bg-blue-50/50 p-4 rounded-xl border border-blue-150 dark:border-blue-900/30 dark:bg-blue-950/10">
              <div className="space-y-1">
                <h4 className="font-bold text-xs text-blue-700 dark:text-blue-400 flex items-center gap-1">
                  <Volume2 className="h-4 w-4" /> Voice Dictation Transcriber
                </h4>
                <p className="text-[10px] text-slate-550 dark:text-slate-400">Transcribe voice consultations to notes instantly.</p>
              </div>
              <button
                type="button"
            ...
            {/* Symptoms */}
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-500">Chief Symptoms</label>
              <textarea
                {...register('symptoms')}
                rows={3}
                placeholder="Describe current medical complaints..."
            ...
            <div className="space-y-1">
              ...
              <textarea
                {...register('diagnosis')}
                rows={2}
                placeholder="Identify suspected illness or pathology..."
                className="w-full rounded-xl border border-slate-200 bg-transparent px-3 py-2 text-xs text-slate-800 dark:border-slate-800 dark:text-slate-100 focus:outline-none"
              />
              {errors.diagnosis && (
            ...
            {/* Clinical Notes */}
            <div className="space-y-1">
              <label className="text-xs font-semibold text-slate-500">Clinical / Action Notes</label>
              <textarea
                {...register('clinicalNotes')}
                rows={3}
                placeholder="Lifestyle recommendations, warning flags, or dosage rationale..."
                className="w-full rounded-xl border border-slate-200 bg-transparent px-3 py-2 text-xs text-slate-800 dark:border-slate-800 dark:text-slate-100 focus:outline-none"
              />
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 space-y-5">
            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-850 pb-3">
              <h3 className="font-extrabold text-sm text-slate-900 dark:text-slate-50 uppercase tracking-wider">Prescription Builder</h3>
              <button
                type="button"
            ...
            {fields.map((field, idx) => (
              <div key={field.id} className="grid gap-3 md:grid-cols-5 bg-slate-50 dark:bg-slate-950 p-3.5 rounded-xl border border-slate-150 dark:border-slate-850 items-end">
                <div className="space-y-1 col-span-2">
                  <label className="text-[10px] font-bold text-slate-400">Medicine Choice</label>
                  <select
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400">Dosage Pattern</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400">Duration (Days)</label>
                  <input
                    type="number"
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-blue-150 bg-blue-50/50 p-5 dark:border-blue-900/30 dark:bg-blue-950/10 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-blue-700 dark:text-blue-400">
                ...
                <h4 className="font-extrabold text-xs uppercase tracking-wider">Clinical Prescribing Assist</h4>
              </div>
              <button
                type="button"
            ...
            {aiSummary ? (
              <div className="space-y-3">
                ...
                <button
                  ...
                  className="text-[10px] font-extrabold text-blue-650 dark:text-blue-400 hover:underline flex items-center gap-1"
                >
                  Apply AI recommendation plan
                </button>
              </div>
            ) : (
              <p className="text-[10px] text-slate-450 leading-relaxed">
                Click &quot;Suggest Plan&quot; to analyze patient chart, allergies and clinical data using Gemini Clinical Intelligence.
              </p>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4">
            <h4 className="font-extrabold text-xs text-slate-900 dark:text-slate-50 uppercase tracking-wider">Lab Recommendations</h4>
          ...
          {/* Follow-up Planner */}
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4">
            <h4 className="font-extrabold text-xs text-slate-900 dark:text-slate-50 uppercase tracking-wider">Follow-up Scheduling</h4>
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-400 block uppercase">Re-visit Date</label>
              <input
                type="date"
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-3">
            ...
            >
              {isSubmitting || saveMutation.isPending ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" /> Saving Consultation...
                </>
              ) : (
            ...
            <Link
              href="/dashboard/doctor"
              className="w-full text-center rounded-xl border border-slate-200 hover:bg-slate-50 dark:border-slate-850 dark:hover:bg-slate-800 text-slate-650 dark:text-slate-350 py-2.5 text-xs font-bold block transition"
            >
              Cancel
            </Link>
          </div>
