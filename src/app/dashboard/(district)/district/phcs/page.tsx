...
export default function DistrictPHCsPage() {const { t } = useLanguage();
  const { user } = useAuth();
  ...
  const list = (facilities || []).filter((f) => f.type === 'phc');

  return (
    <div className="space-y-6">
      <PageHeader
        title={t("district.primary_health_centers_phcs")}
        description={t("district.audit_primary_clinics_and_rural_health_outposts_monitoring_general_patient_outreach")} />
        ...
        <table className="w-full text-left border-collapse text-xs font-semibold text-slate-700 dark:text-slate-350">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-150 dark:border-slate-850 text-slate-450 text-[10px] uppercase font-bold">
              <th className="p-4">{t("district.phc_center_name")}</th>
