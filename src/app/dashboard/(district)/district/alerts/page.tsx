...
export default function DistrictAlertsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="District Critical Health Alerts"
        description="Verify active clinical emergencies, power supply/equipment anomalies, and resource deficits."
      />
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {list.length === 0 ? (
          <div className="md:col-span-2 rounded-2xl border border-slate-200 bg-white p-12 text-center dark:border-slate-800 dark:bg-slate-900">
            ...
            <p className="text-sm font-bold text-slate-800 dark:text-slate-100">All Systems Normal</p>
            <p className="text-xs text-slate-450 mt-1">No critical facility operational warnings active today.</p>
          </div>
        ) : (
          list.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className="border-t border-slate-100 dark:border-slate-850 pt-3 text-[9px] text-slate-400 font-bold flex justify-between">
                  <span>Logged at: {new Date(item.timestamp).toLocaleTimeString()}</span>
                  <span className="text-blue-500 cursor-pointer">Dispatch Emergency Route</span>
                </div>
              </div>
            );
          })
