...
export default function DistrictMedicineMonitoringPage() {
  return (
    <div className="space-y-6">
      <PageHeader
        title="District Medicine Inventory Monitoring"
        description="Heatmap tracking, stock shortages, active allocations, and replenishment predictions."
      />
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4">
          <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
            <Layers className="h-4.5 w-4.5 text-blue-500" /> District Inventory Stock Density
          </h3>
          ...
          <div className="grid grid-cols-5 gap-2.5">
            {Array.from({ length: 25 }).map((_, idx) => {
              ...
              return (
                ...
                >
                  <span className="text-[7px] font-black tracking-widest uppercase">Grid {idx + 101}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex gap-4 text-[9px] font-bold text-slate-500 pt-2 justify-end">
            <div className="flex items-center gap-1"><div className="h-2 w-2 rounded bg-emerald-500" /> Healthy (80%+)</div>
            <div className="flex items-center gap-1"><div className="h-2 w-2 rounded bg-amber-500" /> Warning (30%-80%)</div>
            <div className="flex items-center gap-1"><div className="h-2 w-2 rounded bg-red-500" /> Critical (10%-30%)</div>
            <div className="flex items-center gap-1"><div className="h-2 w-2 rounded bg-red-650" /> Depleted (0%)</div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4">
          <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
            <Battery className="h-4.5 w-4.5 text-emerald-500" /> Top Consumed Formulations
          </h3>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4">
          <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
            <AlertTriangle className="h-4.5 w-4.5 text-red-500" /> Alert-Triggered Shortages
          </h3>
            ...
            <table className="w-full text-left text-xs font-semibold text-slate-700 dark:text-slate-350">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-850 text-[10px] text-slate-400 font-bold uppercase">
                  ...
                  <th className="pb-2">Location Node</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                {mockLowStock.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/20">
                    ...
                    <td className="py-2.5 text-center">{item.current} units</td>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4">
          <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
            <Compass className="h-4.5 w-4.5 text-purple-500" /> AI Stock Replenishment Forecasts
          </h3>
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-xl border border-purple-100 bg-purple-50/10 dark:border-purple-900/20 dark:bg-purple-950/5 space-y-1">
              <h4 className="font-bold text-xs text-purple-800 dark:text-purple-400">West Block CHC Shortage Warning</h4>
              <p className="text-[10px] text-slate-500 leading-relaxed font-semibold">
                Predictive AI indicates 94% chance of oral suspension antibiotics going stock-out by Wednesday evening due to localized monsoonal viral shifts.
              </p>
              <div className="flex gap-2 items-center text-[9px] font-bold text-purple-650 mt-2 cursor-pointer">
                <span>Configure auto-redistribution route</span> <ArrowRight className="h-3.5 w-3.5" />
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl border border-indigo-100 bg-indigo-50/10 dark:border-indigo-900/20 dark:bg-indigo-950/5 space-y-1">
              <h4 className="font-bold text-xs text-indigo-800 dark:text-indigo-400">Metro PHC Center Insulin Forecast</h4>
              <p className="text-[10px] text-slate-500 leading-relaxed font-semibold">
                Predictive AI forecasts consistent consumption shifts; safety stock parameters should be adjusted up by 15% starting next cycle.
              </p>
            </div>
          </div>
        </div>
