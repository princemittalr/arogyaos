  ...
  return (
    ...
    >
      <PageHeader
        ...
        description="Search real-time diagnostic laboratories, emergency care hubs, and local government dispensaries."
      />

      {/* Search & Filter Bar */}
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <SearchBar value={searchQuery} onChange={setSearchQuery} placeholder="Search facility registry..." />
        <FilterBar
          options={[
            {
              key: 'district',
              ...
              choices: [
                { label: 'Bengaluru', value: 'dist_bengaluru' },
                { label: 'Chennai', value: 'dist_chennai' },
                { label: 'Hyderabad', value: 'dist_hyderabad' },
              ],
            },
          ]}
        ...
        <div className="lg:col-span-2 space-y-4">
          {filteredHospitals && filteredHospitals.length > 0 ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-4 text-xs font-semibold pt-1">
                      <span className="text-slate-500">
                        ICU Beds: <span className="text-slate-800 dark:text-slate-200">{capacity.icuBeds} Free</span>
                      </span>
                      <span className="text-slate-500">
                        General: <span className="text-slate-800 dark:text-slate-200">{capacity.generalBeds} Free</span>
                      </span>
                    </div>
                  </div>
            ...
            <EmptyState
              ...
              description="Refine your keyword search or change the location filter options."
            ...
            {selectedHospDetails && currentCapacity ? (
              ...
              >
                ...
                <div className="space-y-2 border-t border-slate-100 dark:border-slate-850 pt-4">
                  <h5 className="font-bold text-xs text-slate-500 uppercase tracking-wide">Bed Status</h5>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-100 dark:border-slate-900">
                      <span className="text-[10px] font-semibold text-slate-450 block">ICU Beds</span>
                      <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">
                        {currentCapacity.icuBeds} Available
                      </span>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-100 dark:border-slate-900">
                      <span className="text-[10px] font-semibold text-slate-450 block">General Ward</span>
                      <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">
                        {currentCapacity.generalBeds} Available
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {/* Departments list */}
                <div className="space-y-2 border-t border-slate-100 dark:border-slate-850 pt-4">
                  <h5 className="font-bold text-xs text-slate-500 uppercase tracking-wide">Departments</h5>
                ...
                {/* Tests available */}
                <div className="space-y-2 border-t border-slate-100 dark:border-slate-850 pt-4">
                  <h5 className="font-bold text-xs text-slate-500 uppercase tracking-wide">Available Tests</h5>
                ...
                <div className="space-y-2.5 border-t border-slate-100 dark:border-slate-850 pt-4">
                  ...
                  <div className="space-y-2">
                    {hospDoctors && hospDoctors.length > 0 ? (
                      ...
                      <p className="text-xs text-slate-450">No doctors listed.</p>
                    )}
                  </div>
                </div>
              ...
              <div className="rounded-2xl border border-dashed border-slate-200 p-6 dark:border-slate-800 text-center py-16 text-slate-400">
                ...
                <p className="text-xs font-semibold">Select a medical facility to view bed counts and specialty clinics.</p>
              </div>
            )}
          </AnimatePresence>
        </div>
