  ...
  return (
    ...
    >
      <PageHeader
        ...
        description="Check schedule tokens, update date availability, or request cancellations."
      />
              ...
              <div className="space-y-3.5">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right space-y-1">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 block">Token #{appt.tokenNumber}</span>
                  </div>
                </div>
                ...
                <div className="rounded-xl bg-slate-50 dark:bg-slate-950 p-3 text-xs border border-slate-100 dark:border-slate-900 space-y-1.5 font-semibold text-slate-650 dark:text-slate-400">
                  ...
                  <p>Time Slot: {appt.appointmentTime}</p>
                </div>
              </div>
              ...
              {activeTab === 'upcoming' && (
                <div className="flex justify-end gap-2 border-t border-slate-100 dark:border-slate-850 pt-3">
                  <button
                    ...
                    className={cn(componentStyles.button.base, componentStyles.button.outline, 'px-3 py-1.5 text-xs')}
                  >
                    Reschedule
                  </button>
                  <button
                    onClick={() => setCancellingApptId(appt.appointmentId)}
                    className="rounded-lg border border-red-200 text-red-650 bg-white hover:bg-red-50 dark:border-red-950/20 dark:bg-slate-900 dark:hover:bg-red-950/20 px-3 py-1.5 text-xs font-semibold transition"
                  >
                    Cancel
                  </button>
                </div>
      ...
      <ConfirmationDialog
        ...
        description="Are you sure you want to cancel this scheduled consultation? The allocated token number will be released."
        isDestructive
      ...
      <AnimatePresence>
        {reschedulingApptId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <h4 className="font-extrabold text-base text-slate-900 dark:text-slate-50">Reschedule Consultation</h4>
              <form onSubmit={handleRescheduleSubmit} className="space-y-4">
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-500">Pick New Date</label>
                  <input
                    type="date"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-slate-500">Pick Time Slot</label>
                  <select
