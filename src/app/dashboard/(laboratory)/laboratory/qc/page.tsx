...
export default function LaboratoryQcPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useLaboratoryModuleData(workerId, 'qc');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Monitor daily QC logs and machine calibration status." 
      />
      {!data || data.length === 0 ? (
