...
export default function LaboratoryProcessingPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useLaboratoryModuleData(workerId, 'processing');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Monitor samples currently being processed by analyzers." 
      />
      {!data || data.length === 0 ? (
