...
export default function LaboratorySyncPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useLaboratoryModuleData(workerId, 'sync');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Offline Sync" 
        description="Manage offline report uploads and pending sync tasks." 
      />
      {!data || data.length === 0 ? (
