...
export default function LaboratorySamplesPage() {
  const { user } = useAuth();
  const workerId = user?.uid || 'worker-1';
  const { data, isLoading } = useLaboratoryModuleData(workerId, 'samples');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Track sample collection status and pending tasks." 
      />
      {!data || data.length === 0 ? (
