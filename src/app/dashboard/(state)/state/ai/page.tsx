    ...
    return (
      <div className="space-y-6">
        <PageHeader 
          ...
          description="State-level predictive analytics and automated recommendations." 
        />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        { districts.map((item: any, i: number) => (
          <div key={i} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
            ...
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">{item.name || item.title || item.resource || 'AI Command Center'}</h3>
