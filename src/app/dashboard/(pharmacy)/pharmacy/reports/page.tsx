...
export default function PharmacyReportsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Pharmacy Analytics & Reports"
        description="Audit stock turnover movement, generate compliance expiry audits, and export logistics files."
      />
      ...
      <div className="grid gap-6 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-3">
          <h3 className="font-extrabold text-[10px] text-slate-450 uppercase tracking-wider mb-2">Report Category</h3>
          ...
          >
            <TrendingUp className="h-4 w-4" /> Stock Movement Audit
          </button>
          ...
          >
            <AlertTriangle className="h-4 w-4" /> Low Stock Alerts Report
          </button>
          ...
          >
            <FileText className="h-4 w-4" /> Batch Expiry Register
          </button>
        </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 space-y-5">
            <div className="flex justify-between items-center border-b border-slate-150 dark:border-slate-800 pb-4">
              <div>
                ...
                <p className="text-[10px] text-slate-450 font-semibold mt-0.5">Hospital: City General Hospital &bull; Generated today</p>
              </div>
                ...
                <table className="w-full text-left text-xs font-semibold text-slate-700 dark:text-slate-300">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-150 dark:border-slate-850 text-slate-450 text-[10px] uppercase font-bold">
                      ...
                      <th className="p-3">Total Dispensed</th>
                      <th className="p-3">Current Stock</th>
                      <th className="p-3">Rotation Ratio</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                    {items.map((i, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-3">{(idx + 2) * 20} units</td>
                        <td className="p-3">{i.quantity} units</td>
                        <td className="p-3 text-emerald-600 dark:text-emerald-400 font-bold">Healthy Rotation</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {selectedReport === 'lowstock' && (
              <div className="overflow-x-auto">
                {lowStockItems.length === 0 ? (
                  <p className="text-xs text-slate-450 italic text-center py-6">All formula stocks are above safety limits.</p>
                ) : (
                  <table className="w-full text-left text-xs font-semibold text-slate-700 dark:text-slate-300">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-150 dark:border-slate-850 text-slate-450 text-[10px] uppercase font-bold">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                      {lowStockItems.map((i) => (
                        <tr key={i.inventoryId} className="hover:bg-slate-50/50">
                          ...
                          <td className="p-3 text-red-600 font-bold">{i.minimumStock - i.quantity} units</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {selectedReport === 'expiry' && (
              <div className="overflow-x-auto">
                {expiredItems.length === 0 ? (
                  <p className="text-xs text-slate-450 italic text-center py-6">No expired items registered on active shelves.</p>
                ) : (
                  <table className="w-full text-left text-xs font-semibold text-slate-700 dark:text-slate-300">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-150 dark:border-slate-850 text-slate-450 text-[10px] uppercase font-bold">
