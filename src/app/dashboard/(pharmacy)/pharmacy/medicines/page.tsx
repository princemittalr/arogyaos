...
export default function MedicinesCatalogPage() {const { t } = useLanguage();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("pharmacy.medicine_formulation_catalog")}
        description={t("pharmacy.search_active_compound_descriptions_strength_variants_and_safety_contraindications")} />
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div key={med.id} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 flex flex-col justify-between gap-4">
            <div className="space-y-3">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <span className="text-[9px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-md inline-block mt-1">{t("pharmacy.strength")}
                  {med.strength}
                  </span>
                </div>
            ...
            <div className="border-t border-slate-100 dark:border-slate-850 pt-3 text-[10px] text-red-650 dark:text-red-400 font-bold flex items-start gap-1.5 bg-red-50/20 dark:bg-red-950/5 p-2 rounded-lg">
              ...
              <span>{t("pharmacy.contraindication")}{med.warnings}</span>
            </div>
          </div>
        )}
      </div>
