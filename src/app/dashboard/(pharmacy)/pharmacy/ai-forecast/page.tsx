...
export default function PharmacyAIForecastPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="AI Medicine Stock Forecast"
        description="Gemini AI analyses consumption velocity, seasonal trends, and safety thresholds to predict future stock shortages."
      />
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4">
        <div className="flex justify-between items-center">
          <h3 className="font-extrabold text-xs uppercase tracking-wider text-slate-900 dark:text-slate-50 flex items-center gap-1.5">
            <AlertTriangle className="h-4.5 w-4.5 text-amber-500" /> Current Inventory Snapshot
          </h3>
          <button
          ...
          >
            {isLoading ? (
              <><RefreshCcw className="h-3.5 w-3.5 animate-spin" /> Analysing...</>
            ) : (
              <><Sparkles className="h-3.5 w-3.5" /> Run AI Forecast</>
            )}
          </button>
        </div>
          ...
          <table className="w-full text-left text-xs font-semibold text-slate-700 dark:text-slate-350">
            <thead>
              <tr className="border-b border-slate-150 dark:border-slate-850 text-[10px] text-slate-400 font-bold uppercase">
                ...
                <th className="pb-2.5 text-right">Min. Level</th>
                <th className="pb-2.5 text-right">Weekly Usage</th>
              ...
              {SAMPLE_INVENTORY.map((item) => {
                ...
                return (
                  <tr key={item.medicineName} className="hover:bg-slate-50/50 dark:hover:bg-slate-850/20">
                    ...
                    <td className="py-3 text-right font-bold">{item.quantity} units</td>
                    <td className="py-3 text-right">{item.minimumStock} units</td>
                    <td className="py-3 text-right">{item.weeklyConsumption}/wk</td>
              ...
              <div key={f.medicineName} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-3.5 hover:border-slate-300 dark:hover:border-slate-750 transition">
                ...
                <div className="grid grid-cols-2 gap-2.5 text-[10px]">
                  <div className="bg-slate-50 dark:bg-slate-950/30 p-2.5 rounded-lg border border-slate-100 dark:border-slate-850">
                    <p className="text-[8px] text-slate-400 uppercase font-semibold">Shortage Date</p>
                    <p className="font-extrabold text-slate-900 dark:text-slate-50 mt-0.5">{f.expectedShortageDate}</p>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-950/30 p-2.5 rounded-lg border border-slate-100 dark:border-slate-850">
                    <p className="text-[8px] text-slate-400 uppercase font-semibold">Refill Needed</p>
                    <p className="font-extrabold text-slate-900 dark:text-slate-50 mt-0.5">{f.recommendedRefillQuantity} units</p>
                  </div>
                </div>
      ...
      {!isLoading && !forecasts && (
        <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 dark:border-slate-800 dark:bg-slate-900 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-xs font-bold text-slate-600 dark:text-slate-400">Run AI Forecast to view predictions</p>
          <p className="text-[10px] text-slate-400 font-semibold">Click the button above to analyse inventory with Gemini AI.</p>
        </div>
      )}
    </div>
  );
