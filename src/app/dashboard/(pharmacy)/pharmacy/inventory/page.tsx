...
export default function PharmacyInventoryPage() {
  ...
  const onSubmit = async (values: InventoryFormValues) => {
    try {
      ...
      toast.error('Failed to register stock updates.');
    }
  };
  ...
  const handleDelete = async (inventoryId: string) => {
    if (confirm('Delete this medicine formulation from active inventory list?')) {
      try {
        ...
        toast.error('Deletion cancelled due to transaction errors.');
      }
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <PageHeader
          ...
          description="Manage hospital medicine formula batches, supplier logistics, and warning parameters."
        />
        <button
          onClick={() => {
            setEditingItem(null);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 space-y-4 max-w-3xl">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-450 uppercase block">Medicine Name</label>
              <input
                type="text"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-450 uppercase block">Batch Number</label>
              <input
                type="text"
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-450 uppercase block">Initial Stock Qty</label>
              <input
                type="number"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-450 uppercase block">Min Alert Limit</label>
              <input
                type="number"
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-450 uppercase block">Supplier</label>
              <input
                type="text"
                {...register('supplier')}
                placeholder="Astra Biotech"
                className="w-full rounded-xl border border-slate-200 bg-transparent px-3 py-2 text-xs text-slate-800 dark:border-slate-800 dark:text-slate-100 focus:outline-none"
              />
          ...
          <div className="flex justify-end pt-3">
            <button
              ...
              className="rounded-xl bg-blue-650 hover:bg-blue-700 disabled:bg-blue-400 text-white px-5 py-2 text-xs font-bold transition"
            >
              Commit Stock Parameters
            </button>
          </div>
        </form>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-4 bg-white p-4 rounded-2xl border border-slate-200 dark:bg-slate-900 dark:border-slate-800">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search stock by formula name or batch..."
            value={searchTerm}
      ...
      {filteredItems.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="text-sm font-bold text-slate-800 dark:text-slate-100">No matching stock found</p>
          <p className="text-xs text-slate-450 mt-1">Add items or broaden your filter search.</p>
        </div>
      ) : (
                ...
                return (
                  <tr key={item.inventoryId} className="hover:bg-slate-50/50 dark:hover:bg-slate-850/20">
                    <td className="p-4">
                      ...
                      <p className="text-[10px] text-slate-400">Batch: {item.batchNumber}</p>
                    </td>
                    ...
                    <td className="p-4">
                      {isExpired ? (
                        <span className="bg-red-100 text-red-700 dark:bg-red-950/20 dark:text-red-400 px-2 py-0.5 rounded-md text-[9px] font-bold">Expired</span>
                      ) : isOut ? (
                        <span className="bg-red-50 text-red-650 dark:bg-red-950/10 dark:text-red-400 px-2 py-0.5 rounded-md text-[9px] font-bold">Out of stock</span>
                      ) : isLow ? (
                        <span className="bg-amber-100 text-amber-700 dark:bg-amber-950/20 dark:text-amber-400 px-2 py-0.5 rounded-md text-[9px] font-bold">Low stock</span>
                      ) : (
                        <span className="bg-emerald-100 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-400 px-2 py-0.5 rounded-md text-[9px] font-bold">Healthy</span>
                      )}
                    </td>
