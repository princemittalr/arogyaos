...
[![Gemini AI](https://img.shields.io/badge/Google-Gemini_2.5-4285F4?style=for-the-badge&logo=google)](https://ai.google.dev/)
...
```mermaid
flowchart TD
    A[Citizens / Doctors / Admins] --> B["Next.js 15 Frontend<br/>(App Router + React + TypeScript)"]
    ...
    D --> F
    E --> F
    F --> G[Google Gemini AI APIs]
    G --> H["AI Insights • Predictions • Recommendations"]
```
...
```mermaid
flowchart TD
    A[Presentation Layer] --> B[Dashboard Shell]
    B --> C[Feature Modules]
    C --> D["Hooks (TanStack Query)"]
    D --> E[Service Layer]
    E --> F[Firebase SDK]
...
flowchart TD
    ...
    NextApp --> Auth[Firebase Authentication]
    NextApp --> Firestore[Cloud Firestore]
    NextApp --> Storage[Firebase Storage]
    NextApp --> Gemini[Gemini AI]
    Gemini --> Intelligence[Healthcare Intelligence]
```
